...
public class EventTodoController {
    ...
    public EventTodoController(EventServiceApi eventService) {
        this.eventService = eventService;
    }

    @PostMapping("/addTodo")
    public PojoEvent addTodo(@RequestParam(value = "eventId") long eventId,
                             @RequestParam(value = "userIds", defaultValue = "") List<UUID> userIds,
                             @RequestParam(value = "done", defaultValue = "false") boolean isDone,
                             @RequestBody LightPojoTodoEntry todoEntry) {
        return eventService.addTodo(eventId, todoEntry, userIds, isDone);
    }
    ...
    public PojoEvent addTodoUsers(@RequestParam(value = "eventId") long eventId,
                                  @RequestParam("todoName") String todoName,
                                  @RequestParam(value = "userIds", required = false) List<UUID> userIdList) {
        return eventService.addTodoUsers(eventId, todoName, userIdList);
    }

    @PostMapping("/removeUsers")
    public PojoEvent removeTodoUsers(@RequestParam(value = "eventId") long eventId,
                                     @RequestParam("todoName") String todoName,
                                     @RequestParam(value = "userIds", required = false) List<UUID> userIdList) {
    ...
    @GetMapping("/findByTodoId")
    public PojoEvent findByTodo(@RequestParam("todoId") long todoId) {
        return eventService.findEventFromTodoId(todoId);
    }
